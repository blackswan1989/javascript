...
> 위와 같은 방식 처럼 '객체 리터럴 표기법'이란 중괄호 {}안에 '속성 이름 : 값'을 하나의 쌍으로 지정한다. 속성이 여러 개일 경우 쉼표(,)를 넣어 구분해준다. 함수 또한 '함수 이름: '의 다음에 함수를 정의하며 '함수이름 : function() {}'형식을 사용한다. 추가로 객체를 정의해 변수에 할당할 때에는 닫는 중괄호 })다음에 세미콜론(;)을 붙여 준다.
